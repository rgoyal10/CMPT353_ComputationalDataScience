...
def pivot_months_pandas(data):
    ...
    temp = data.groupby(['name','month'])
    
    monthly = temp.aggregate('sum').reset_index()
    counts = temp.aggregate('count').reset_index()
    
    
    monthly = monthly.pivot('name' , 'month', 'precipitation')
    counts = counts.pivot('name' , 'month', 'precipitation')
